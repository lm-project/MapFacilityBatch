...
/**
 * 输入 AN分图幅数据 AN删除的数据 BD新增 step6 step4 1 : 1 替换 step7 查找POI
 */
public class WashDataMergeForReplace extends AbstractWashDataMerge {

	private Logger mergeLog = Logger.getLogger("mergeLogger");

	private Collection<File> anShpFiles = new ArrayList<File>();
	private String meshDir; //an 分幅路径
	...
	public WashDataMergeForReplace() {
		super();
	...
	@Override
	public void merge() {
		mergeLog.debug("<----------------------替换融合开始！ ---------------------->");
		findShpFiles(meshDir, anShpFiles);
		...
		for (File file : anShpFiles) {
			...
			try {
				...
				mergeLog.info(file.getParentFile().getName() + "图幅的数据数量:" + count(anMergeTable));
				createInteractionAn(anDelTable, anMergeTable, interactDelAn);
				mergeLog.debug("此图幅与替换融合的an交集的数量："+count(interactDelAn, false));
				...
				String repalaceMergePath = anMergeDir+"/替换融合/"+ ss[ss.length - 4] + "/" + ss[ss.length - 3] + "/" + ss[ss.length - 2] + "/"  + ss[ss.length - 1];
